...
app.post('/submit-attendance', async (req, res) => {
  ...
  try {
    const newAttendance = new AttendanceModel({ attendanceRecords, date: new Date(date) ,date });
    await newAttendance.save();
    res.status(201).json({ message: 'Attendance data submitted successfully.' });
  } catch (error) {
...
app.get('/get-attendence', async(req,res)=> {
  ...
  //AttendanceModel.findById(req.params._id)
...
app.post('/register', async (req, res) => {
    try {
      const { username, email, password, UserType ,phone,course,address} = req.body;
      ...
      const user = new UserModel1({ username, email, password: hashPassword , UserType , phone ,course, address}).save();
      // await user.save();
  ...
  app.post('/login', async (req, res) => {
    try {
      ...
      //   return res.status(401).json({ message: 'Incorrect password.' });
      // }
      // const user01 = await UserModel1.findOne({ UserType });
      // if (!user01) {
      //   return res.status(404).json({ message: 'Usertype is not define' });
      // }
